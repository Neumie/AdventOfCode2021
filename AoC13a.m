...
numberOfPoints = 782; %18 v testu 782
numberOfFolds = 12; %2 v testu 12

data = readmatrix('inputAoC13.txt', Delimiter = ',', OutputType = 'string');
...
for i = 1:12 %how many folds to do
    if (sign(folds(i)) == -1) %Y fold
        ...
        for j = 1:height(tempMap) %výškakrát
            for k = 1:width(tempMap) %šířkakrát
                if (tempMap(j, k) == 1)
                    map(j, (width(map)+1)-k) = 1;
                end
            end
        end
    end
end %druhá část tohoto dne jen se kouknout na mapu a přečíst co tam je napsáno. -> PFKKLCFP